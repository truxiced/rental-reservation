...
const LocalizationProvider = exports.LocalizationProvider = function LocalizationProvider(inProps) {
  ...
  const adapter = React.useMemo(() => {
    ...
    if (!dateAdapter.isMUIAdapter) {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Date Pickers: The date adapter should be imported from `@mui/x-date-pickers` or `@mui/x-date-pickers-pro`, not from `@date-io`. ' + 'Using adapters from @date-io directly is not supported. ' + "For example, use `import { AdapterDayjs } from '@mui/x-date-pickers/AdapterDayjs'` instead of `import AdapterDayjs from '@date-io/dayjs'`. " + 'See https://mui.com/x/react-date-pickers/quickstart/#installation' : (0, _formatErrorMessage2.default)(143));
    }
    return dateAdapter;
